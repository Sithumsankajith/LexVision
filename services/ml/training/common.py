...
class TrainingTarget:
    ...
    default_version: str
    default_imgsz: int
    supported_imgsz: tuple[int, ...] | None
    default_patience: int
    description: str
    default_train_overrides: dict[str, Any]
    traffic_augmentation_profile: dict[str, Any]
...
TRAINING_TARGETS: dict[str, TrainingTarget] = {
    "helmet": TrainingTarget(
        ...
        traffic_augmentation_profile={
            "blur": {"p": 0.08, "blur_limit": 5},
            "median_blur": {"p": 0.03, "blur_limit": 5},
            "motion_blur": {"p": 0.08, "blur_limit": 7},
            "brightness_contrast": {"p": 0.2, "brightness_limit": 0.18, "contrast_limit": 0.18},
            "clahe": {"p": 0.05},
            "gamma": {"p": 0.08, "gamma_limit": [80, 120]},
            "compression": {"p": 0.05, "quality_lower": 70},
        },
    ),
    "anpr": TrainingTarget(
        key="anpr",
        ...
        traffic_augmentation_profile={
            "blur": {"p": 0.12, "blur_limit": 5},
            "median_blur": {"p": 0.04, "blur_limit": 5},
            "motion_blur": {"p": 0.14, "blur_limit": 9},
            "brightness_contrast": {"p": 0.24, "brightness_limit": 0.2, "contrast_limit": 0.25},
            "clahe": {"p": 0.06},
            "gamma": {"p": 0.1, "gamma_limit": [75, 125]},
            "compression": {"p": 0.08, "quality_lower": 65},
        },
    ),
}
...
def render_report_markdown(summary: dict[str, Any]) -> str:
    lines = [
        ...
        f"- Deterministic: {summary['training'].get('deterministic')}",
        f"- Optimizer: `{summary['training'].get('optimizer', 'auto')}`",
        f"- Early stopping patience: {summary['training'].get('patience')}",
        f"- Learning rate: {summary['training'].get('lr0')}",
        f"- Final LR factor: {summary['training'].get('lrf')}",
        f"- Weight decay: {summary['training'].get('weight_decay')}",
        f"- Close mosaic: {summary['training'].get('close_mosaic')}",
        f"- Cosine LR: {summary['training'].get('cos_lr')}",
        f"- Traffic Albumentations active: {summary['training'].get('traffic_augmentation', {}).get('active') if summary['training'].get('traffic_augmentation') else False}",
        "",
        "## Metrics",
        "",
    ]
